defmodule Alchevents.Registry.ChannelTest do
  use ExUnit.Case
  alias Alchevents.Registry.Channel

  setup do
    Registry.unregister_match(Alchevents.Registry.Channel, :channel, [])
    Registry.unregister_match(Alchevents.Registry.Channel, :channel1, [])
    Registry.unregister_match(Alchevents.Registry.Channel, :channel2, [])
    Registry.unregister_match(Alchevents.Registry.Channel, :channel3, [])
  end

  describe "#register" do
    test "register a new topic if no topic is registered for the same channel" do
      assert Registry.lookup(Alchevents.Registry.Channel, :channel) == []
      assert :ok == Channel.register(:channel)
      assert Registry.lookup(Alchevents.Registry.Channel, :channel) == [{self(), []}]
    end
